...
public class Texture implements Disposable {
    ...
    public Texture(String file) {
        ...
        ByteBuffer pixelsBuffer = STBImage.stbi_load_from_memory(ioResourceToByteBuffer(file), widthBuffer, heightBuffer, glPixelFormatBuffer, 4);

        init(widthBuffer.get(), heightBuffer.get(), pixelsBuffer, glPixelFormatBuffer.get());
    }
    ...
    public void set(int width, int height, float[] pixels) {
        byte[] bytes = new byte[pixels.length];
        for (int i = 0; i < pixels.length; i++) {
            bytes[i] = (byte) (pixels[i] * 255f);
        }
        set(width, height, allocate(bytes), glPixelFormat);
    }
